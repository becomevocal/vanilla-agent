...
Create a `.env` file alongside this package (or export env vars another way) and set any required secrets (e.g. `TRAVRSE_API_KEY=tv_test_xxx`). The dev server automatically loads `.env` via `dotenv`. The server prefers port `43111` but automatically falls back to the next available port; the Vite demo app proxies `/api/chat/dispatch` to whatever port the proxy logs.
